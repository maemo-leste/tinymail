...
#define d(x)
#define dd(x) (camel_lite_debug("vfolder")?(x):0)
...
static void camel_lite_vee_folder_class_init (CamelVeeFolderClass *klass);
static void camel_lite_vee_folder_init       (CamelVeeFolder *obj);
static void camel_lite_vee_folder_finalise   (CamelObject *obj);
...
static CamelFolderClass *camel_lite_vee_folder_parent;

CamelType
camel_lite_vee_folder_get_type (void)
{
	static CamelType type = CAMEL_INVALID_TYPE;

	if (type == CAMEL_INVALID_TYPE) {
		type = camel_lite_type_register (camel_lite_folder_get_type (), "CamelLiteVeeFolder",
					    sizeof (CamelVeeFolder),
					    sizeof (CamelVeeFolderClass),
					    (CamelObjectClassInitFunc) camel_lite_vee_folder_class_init,
					    NULL,
					    (CamelObjectInitFunc) camel_lite_vee_folder_init,
					    (CamelObjectFinalizeFunc) camel_lite_vee_folder_finalise);
	}
...
void
camel_lite_vee_folder_construct (CamelVeeFolder *vf, CamelStore *parent_store, const char *full, const char *name, guint32 flags)
{
	CamelFolder *folder = (CamelFolder *)vf;

	vf->flags = flags;
	camel_lite_folder_construct(folder, parent_store, full, name);

	folder->summary = camel_lite_vee_summary_new(folder);
 ...
 **/
CamelFolder *
camel_lite_vee_folder_new(CamelStore *parent_store, const char *full, guint32 flags)
{
	CamelVeeFolder *vf;
	char *tmp;
	...
	if (CAMEL_IS_VEE_STORE(parent_store) && strcmp(full, CAMEL_UNMATCHED_NAME) == 0) {
		...
		if (name == NULL)
			...
			name++;
		vf = (CamelVeeFolder *)camel_lite_object_new(camel_lite_vee_folder_get_type());
		camel_lite_vee_folder_construct(vf, parent_store, full, name, flags);
	}

	d(printf("returning folder %s %p, count = %d\n", name, vf, camel_lite_folder_get_message_count((CamelFolder *)vf)));

	tmp = g_strdup_printf("%s/%s.cmeta", ((CamelService *)parent_store)->url->path, full);
	camel_lite_object_set(vf, NULL, CAMEL_OBJECT_STATE_FILE, tmp, NULL);
	g_free(tmp);
	if (camel_lite_object_state_read(vf) == -1) {
...
void
camel_lite_vee_folder_set_expression(CamelVeeFolder *vf, const char *query)
{
	((CamelVeeFolderClass *)((CamelObject *)vf)->klass)->set_expression(vf, query);
}

/**
 * camel_lite_vee_folder_add_folder:
 ...
 **/
void
camel_lite_vee_folder_add_folder(CamelVeeFolder *vf, CamelFolder *sub)
{
	...
	if (g_list_find(p->folders, sub) == NULL) {
		camel_lite_object_ref((CamelObject *)sub);
	...
	d(printf("camel_lite_vee_folder_add_folde(%p, %p)\n", vf, sub));

	camel_lite_object_hook_event((CamelObject *)sub, "folder_changed", (CamelObjectEventHookFunc)folder_changed, vf);
	camel_lite_object_hook_event((CamelObject *)sub, "deleted", (CamelObjectEventHookFunc)subfolder_deleted, vf);
	camel_lite_object_hook_event((CamelObject *)sub, "renamed", (CamelObjectEventHookFunc)folder_renamed, vf);

	((CamelVeeFolderClass *)((CamelObject *)vf)->klass)->add_folder(vf, sub);
}

/**
 * camel_lite_vee_folder_remove_folder:
 * @vf: Virtual Folder object
...
camel_lite_vee_folder_remove_folder(CamelVeeFolder *vf, CamelFolder *sub)
{
	struct _CamelVeeFolderPrivate *p = _PRIVATE(vf);
	int i;
	CamelVeeFolder *folder_unmatched = vf->parent_vee_store ? vf->parent_vee_store->folder_unmatched : NULL;
	...
	camel_lite_object_unhook_event((CamelObject *)sub, "folder_changed", (CamelObjectEventHookFunc) folder_changed, vf);
	camel_lite_object_unhook_event((CamelObject *)sub, "deleted", (CamelObjectEventHookFunc) subfolder_deleted, vf);
	camel_lite_object_unhook_event((CamelObject *)sub, "renamed", (CamelObjectEventHookFunc) folder_renamed, vf);
...
/**
 * camel_lite_vee_folder_rebuild_folder:
 * @vf: Virtual Folder object
 * @sub: source CamelFolder to add to @vf
 * @ex: Exception.
 *
 ...
 **/
int
camel_lite_vee_folder_rebuild_folder(CamelVeeFolder *vf, CamelFolder *sub, CamelException *ex)
{
...
{
	camel_lite_vee_folder_remove_folder(vf, folder);
	camel_lite_object_unref((CamelObject *)folder);
}

/**
 * camel_lite_vee_folder_set_folders:
 * @vf:
 * @folders:
 *
 ...
 **/
void
camel_lite_vee_folder_set_folders(CamelVeeFolder *vf, GList *folders)
{
	struct _CamelVeeFolderPrivate *p = _PRIVATE(vf);
	...
	while (l) {
		...
		camel_lite_object_ref((CamelObject *)l->data);
		l = l->next;
	}
	CAMEL_VEE_FOLDER_UNLOCK(vf, subfolder_lock);
	...
	while (l) {
		if ((folder = g_hash_table_lookup(remove, l->data))) {
			...
			camel_lite_vee_folder_add_folder(vf, l->data);
		}
		l = l->next;
	}
...
/**
 * camel_lite_vee_folder_hash_folder:
 * @folder:
 * @:
 *
...
camel_lite_vee_folder_hash_folder(CamelFolder *folder, char buffer[8])
{
	...
	md5_init(&ctx);
	tmp = camel_lite_service_get_url((CamelService *)folder->parent_store);
...
/**
 * camel_lite_vee_folder_get_location:
 * @vf:
 * @vinfo:
 ...
 **/
CamelFolder *
camel_lite_vee_folder_get_location(CamelVeeFolder *vf, const CamelVeeMessageInfo *vinfo, char **realuid)
{
	CamelFolder *folder;
	...
	if (CAMEL_IS_VEE_FOLDER(folder)) {
		...
		vfinfo = (CamelVeeMessageInfo *)camel_lite_folder_get_message_info(folder, camel_lite_message_info_uid(vinfo)+8);
		res = camel_lite_vee_folder_get_location((CamelVeeFolder *)folder, vfinfo, realuid);
		camel_lite_folder_free_message_info(folder, (CamelMessageInfo *)vfinfo);
		return res;
	} else {
		if (realuid)
			*realuid = g_strdup(camel_lite_message_info_uid(vinfo)+8);
...
static void vee_refresh_info(CamelFolder *folder, CamelException *ex)
{
	...
	while (node) {
		...
		if (camel_lite_vee_folder_rebuild_folder(vf, f, ex) == -1)
			break;
...
vee_sync(CamelFolder *folder, gboolean expunge, CamelException *ex)
{
	...
	while (node) {
		...
		camel_lite_folder_sync(f, expunge, ex);
		if (camel_lite_exception_is_set(ex)) {
			char *desc;

			camel_lite_object_get(f, NULL, CAMEL_OBJECT_DESCRIPTION, &desc, NULL);
			camel_lite_exception_setv(ex, ex->id, _("Error storing `%s': %s"), desc, ex->desc);
			break;
		}

		/* auto update vfolders shouldn't need a rebuild */
		if ((vf->flags & CAMEL_STORE_VEE_FOLDER_AUTO) == 0
		    && camel_lite_vee_folder_rebuild_folder(vf, f, ex) == -1)
			break;
	...
	/* camel_lite_object_state_write(vf); */
}
...
vee_get_message(CamelFolder *folder, const char *uid, CamelFolderReceiveType type, gint param, CamelException *ex)
{
	...
	mi = (CamelVeeMessageInfo *)camel_lite_folder_summary_uid(folder->summary, uid);
	if (mi) {
		msg =  camel_lite_folder_get_message(mi->real->summary->folder, camel_lite_message_info_uid(mi)+8, type, param, ex);
...
vee_search_by_expression(CamelFolder *folder, const char *expression, CamelException *ex)
{
	...
	while (node) {
		...
		if (g_hash_table_lookup(searched, f) == NULL) {
			camel_lite_vee_folder_hash_folder(f, hash);
			/* FIXME: shouldn't ignore search exception */
			matches = camel_lite_folder_search_by_expression(f, expression, NULL);
			if (matches) {
				...
				camel_lite_folder_search_free(f, matches);
			}
	...
	while (node) {
		...
		if (g_hash_table_lookup(searched, f) == NULL) {
			...
			if (folder_uids->len > 0) {
				matches = camel_lite_folder_search_by_uids(f, expression, folder_uids, ex);
				if (matches) {
					for (i = 0; i < matches->len; i++) {
						char *uid = matches->pdata[i], *vuid;
					...
					g_warning("Search failed: %s", camel_lite_exception_get_description(ex));
				}
			}
			g_hash_table_insert(searched, f, f);
		}
...
{
	camel_lite_exception_set(ex, CAMEL_EXCEPTION_SYSTEM, _("Cannot copy or move messages into a Virtual Folder"));
}
...
static void vee_rename(CamelFolder *folder, const char *new)
{
	...
	((CamelFolderClass *)camel_lite_vee_folder_parent)->rename(folder, new);
}
...
static void vee_delete(CamelFolder *folder)
{
	...
	while (p->folders) {
		...
		camel_lite_object_ref(f);
		CAMEL_VEE_FOLDER_UNLOCK(folder, subfolder_lock);

		camel_lite_vee_folder_remove_folder((CamelVeeFolder *)folder, f);
		camel_lite_object_unref(f);
		CAMEL_VEE_FOLDER_LOCK(folder, subfolder_lock);
	}
	CAMEL_VEE_FOLDER_UNLOCK(folder, subfolder_lock);

	((CamelFolderClass *)camel_lite_vee_folder_parent)->delete(folder);
}
...
vee_folder_add_uid(CamelVeeFolder *vf, CamelFolder *f, const char *inuid, const char hash[8])
{
	...
	info = camel_lite_folder_get_message_info(f, inuid);
	if (info) {
		mi = camel_lite_vee_summary_add((CamelVeeSummary *)((CamelFolder *)vf)->summary, info, hash);
		camel_lite_folder_free_message_info(f, info);
	}
	return mi;
}
...
vee_folder_remove_folder(CamelVeeFolder *vf, CamelFolder *source)
{
	...
	if (folder_unmatched != NULL) {
		...
		if ((vf->flags & CAMEL_STORE_FOLDER_PRIVATE) == 0 && !killun) {
			...
			CAMEL_VEE_FOLDER_UNLOCK(folder_unmatched, subfolder_lock);
			camel_lite_vee_folder_hash_folder(source, hash);
		}
		...
		if (killun) {
			start = -1;
			last = -1;
			count = camel_lite_folder_summary_count(((CamelFolder *)folder_unmatched)->summary);
			for (i=0;i<count;i++) {
				CamelVeeMessageInfo *mi = (CamelVeeMessageInfo *)camel_lite_folder_summary_index(((CamelFolder *)folder_unmatched)->summary, i);

				if (mi) {
					if (mi->real->summary == ssummary) {
						camel_lite_folder_change_info_remove_uid(folder_unmatched->changes, camel_lite_message_info_uid(mi));
						if (last == -1) {
							last = start = i;
						} else if (last+1 == i) {
							last = i;
						} else {
							camel_lite_folder_summary_remove_range(((CamelFolder *)folder_unmatched)->summary, start, last);
							i -= (last-start)+1;
							start = last = i;
						}
					}
	...
	for (i=0;i<count;i++) {
		CamelVeeMessageInfo *mi = (CamelVeeMessageInfo *)camel_lite_folder_summary_index(folder->summary, i);
		if (mi) {
			if (mi->real->summary == ssummary) {
				const char *uid = camel_lite_message_info_uid(mi);

				camel_lite_folder_change_info_remove_uid(vf->changes, uid);
				...
				if (last == -1) {
					...
					camel_lite_folder_summary_remove_range(folder->summary, start, last);
					i -= (last-start)+1;
					start = last = i;
				}
				if ((vf->flags & CAMEL_STORE_FOLDER_PRIVATE) == 0 && folder_unmatched != NULL) {
					if (still) {
						if (g_hash_table_lookup_extended(unmatched_uids, uid, (void **)&oldkey, &oldval)) {
							...
							if (n == 1) {
								...
								if (vee_folder_add_uid(folder_unmatched, source, oldkey+8, hash))
									camel_lite_folder_change_info_add_uid(folder_unmatched->changes, oldkey);
								g_free(oldkey);
							} else {
						...
						if (g_hash_table_lookup_extended(unmatched_uids, camel_lite_message_info_uid(mi), (void **)&oldkey, &oldval)) {
							g_hash_table_remove(unmatched_uids, oldkey);
							g_free(oldkey);
						}
					}
	...
	if (folder_unmatched) {
		if (camel_lite_folder_change_info_changed(folder_unmatched->changes)) {
			unmatched_changes = folder_unmatched->changes;
			folder_unmatched->changes = camel_lite_folder_change_info_new();
		}

		CAMEL_VEE_FOLDER_UNLOCK(folder_unmatched, summary_lock);
	}

	if (camel_lite_folder_change_info_changed(vf->changes)) {
		vf_changes = vf->changes;
		vf->changes = camel_lite_folder_change_info_new();
	}

	CAMEL_VEE_FOLDER_UNLOCK(vf, summary_lock);

	if (unmatched_changes) {
		camel_lite_object_trigger_event((CamelObject *)folder_unmatched, "folder_changed", unmatched_changes);
		camel_lite_folder_change_info_free(unmatched_changes);
	}

	if (vf_changes) {
		camel_lite_object_trigger_event((CamelObject *)vf, "folder_changed", vf_changes);
		camel_lite_folder_change_info_free(vf_changes);
	}
}
...
unmatched_check_uid(char *uidin, void *value, struct _update_data *u)
{
	...
	if (n == 0) {
		if (vee_folder_add_uid(u->folder_unmatched, u->source, uidin, u->hash))
			camel_lite_folder_change_info_add_uid(u->folder_unmatched->changes, uid);
	} else {
		CamelVeeMessageInfo *mi = (CamelVeeMessageInfo *)camel_lite_folder_summary_uid(((CamelFolder *)u->folder_unmatched)->summary, uid);
		if (mi) {
			camel_lite_folder_summary_remove(((CamelFolder *)u->folder_unmatched)->summary, (CamelMessageInfo *)mi);
			camel_lite_folder_change_info_remove_uid(u->folder_unmatched->changes, uid);
			camel_lite_message_info_free((CamelMessageInfo *)mi);
		}
	}
}
...
folder_added_uid(char *uidin, void *value, struct _update_data *u)
{
	...
	if ( (mi = vee_folder_add_uid(u->vf, u->source, uidin, u->hash)) ) {
		camel_lite_folder_change_info_add_uid(u->vf->changes, camel_lite_message_info_uid(mi));

		if (!CAMEL_IS_VEE_FOLDER(u->source) && u->unmatched_uids != NULL) {
			if (g_hash_table_lookup_extended(u->unmatched_uids, camel_lite_message_info_uid(mi), (void **)&oldkey, &oldval)) {
				n = GPOINTER_TO_INT (oldval);
				g_hash_table_insert(u->unmatched_uids, oldkey, GINT_TO_POINTER(n+1));
			} else {
				g_hash_table_insert(u->unmatched_uids, g_strdup(camel_lite_message_info_uid(mi)), GINT_TO_POINTER(1));
			}
		}
	}
}
...
vee_rebuild_folder(CamelVeeFolder *vf, CamelFolder *source, CamelException *ex)
{
	...
	if (vf->expression == NULL) {
		...
		match = camel_lite_folder_search_by_expression(f, vf->expression, ex);
		if (match == NULL)
			return -1;
	}
	...
	camel_lite_vee_folder_hash_folder(source, u.hash);
	...
	all = camel_lite_folder_get_uids(f);
	for (i=0;i<all->len;i++)
	...
	for (i=0;i<count;i++) {
		...
		if (mi) {
			if (mi->real->summary == ssummary) {
				char *uid = (char *)camel_lite_message_info_uid(mi), *oldkey;
				void *oldval;
				...
				if (g_hash_table_lookup(matchhash, uid+8) == NULL) {
					...
					camel_lite_folder_change_info_remove_uid(vf->changes, camel_lite_message_info_uid(mi));
	...
	if (folder_unmatched != NULL) {
		...
		for (i=0;i<count;i++) {
			...
			if (mi) {
				if (mi->real->summary == ssummary) {
					char *uid = (char *)camel_lite_message_info_uid(mi);

					if (g_hash_table_lookup(allhash, uid+8) == NULL) {
						/* no longer exists at all, just remove it entirely */
						camel_lite_folder_summary_remove_index(((CamelFolder *)folder_unmatched)->summary, i);
						camel_lite_folder_change_info_remove_uid(folder_unmatched->changes, camel_lite_message_info_uid(mi));
						i--;
					} else {
	...
	if (vf->expression == NULL)
		...
		camel_lite_folder_search_free(f, match);
	camel_lite_folder_free_uids(f, all);
...
folder_changed_add_uid(CamelFolder *sub, const char *uid, const char hash[8], CamelVeeFolder *vf)
{
	...
	vuid = camel_lite_message_info_uid(vinfo);
	camel_lite_folder_change_info_add_uid(vf->changes,  vuid);
	...
	if ((vf->flags & CAMEL_STORE_FOLDER_PRIVATE) == 0 && !CAMEL_IS_VEE_FOLDER(sub) && folder_unmatched != NULL) {
		...
		vinfo = (CamelVeeMessageInfo *)camel_lite_folder_get_message_info((CamelFolder *)folder_unmatched, vuid);
		if (vinfo) {
			camel_lite_folder_change_info_remove_uid(folder_unmatched->changes, vuid);
			camel_lite_folder_summary_remove(((CamelFolder *)folder_unmatched)->summary, (CamelMessageInfo *)vinfo);
			camel_lite_folder_free_message_info((CamelFolder *)folder_unmatched, (CamelMessageInfo *)vinfo);
		}
	}
}
...
folder_changed_remove_uid(CamelFolder *sub, const char *uid, const char hash[8], int keep, CamelVeeFolder *vf)
{
	...
	vinfo = (CamelVeeMessageInfo *)camel_lite_folder_summary_uid(folder->summary, vuid);
	if (vinfo) {
		camel_lite_folder_change_info_remove_uid(vf->changes, vuid);
		camel_lite_folder_summary_remove(folder->summary, (CamelMessageInfo *)vinfo);
		camel_lite_message_info_free((CamelMessageInfo *)vinfo);
	}
	...
	if ((vf->flags & CAMEL_STORE_FOLDER_PRIVATE) == 0 && !CAMEL_IS_VEE_FOLDER(sub) && folder_unmatched != NULL) {
		if (keep) {
			...
			if (vinfo) {
				camel_lite_folder_change_info_remove_uid(folder_unmatched->changes, vuid);
				camel_lite_folder_summary_remove_uid(((CamelFolder *)folder_unmatched)->summary, vuid);
				camel_lite_folder_free_message_info((CamelFolder *)folder_unmatched, (CamelMessageInfo *)vinfo);
			}
		}
	}
...
folder_changed_change_uid(CamelFolder *sub, const char *uid, const char hash[8], CamelVeeFolder *vf)
{
	...
	vinfo = (CamelVeeMessageInfo *)camel_lite_folder_summary_uid(folder->summary, vuid);
	if (folder_unmatched != NULL)
		uinfo = (CamelVeeMessageInfo *)camel_lite_folder_summary_uid(((CamelFolder *)folder_unmatched)->summary, vuid);
	if (vinfo || uinfo) {
		info = camel_lite_folder_get_message_info(sub, uid);
		if (info) {
			if (vinfo) {
				camel_lite_folder_change_info_change_uid(vf->changes, vuid);
				camel_lite_message_info_free((CamelMessageInfo *)vinfo);
			}

			if (uinfo) {
				camel_lite_folder_change_info_change_uid(folder_unmatched->changes, vuid);
				camel_lite_message_info_free((CamelMessageInfo *)uinfo);
			}

			camel_lite_folder_free_message_info(sub, info);
		} else {
			if (vinfo) {
...
folder_changed_change(CamelSession *session, CamelSessionThreadMsg *msg)
{
	...
	camel_lite_vee_folder_hash_folder(sub, hash);
	...
	if (changes->uid_added->len > 0) {
		...
		matches_added = camel_lite_folder_search_by_uids(sub, vf->expression, changes->uid_added, NULL);
	}
	...
	if (changed->len > 0) {
		...
		if (changed->len)
			matches_changed = camel_lite_folder_search_by_uids(sub, vf->expression, changed, NULL);
	}
		...
		for (i=0;i<changes->uid_added->len;i++) {
			...
			if (g_hash_table_lookup(matches_hash, uid)) {
				...
				if (!CAMEL_IS_VEE_FOLDER(sub) && folder_unmatched != NULL && g_hash_table_lookup(unmatched_uids, vuid) == NULL) {
					...
					if (vinfo == NULL) {
						if (vee_folder_add_uid(folder_unmatched, sub, uid, hash))
							camel_lite_folder_change_info_add_uid(folder_unmatched->changes, vuid);
					} else {
						camel_lite_folder_free_message_info((CamelFolder *)folder_unmatched, (CamelMessageInfo *)vinfo);
					}
				}
	...
	if (matches_added)
		camel_lite_folder_search_free(sub, matches_added);

	if (matches_changed)
		camel_lite_folder_search_free(sub, matches_changed);
...
folder_changed_free(CamelSession *session, CamelSessionThreadMsg *msg)
{
	...
	camel_lite_object_unref((CamelObject *)m->vf);
	camel_lite_object_unref((CamelObject *)m->sub);
}
...
folder_changed_base(CamelVeeFolder *vf, CamelFolder *sub, CamelFolderChangeInfo *changes)
{
	...
	m = camel_lite_session_thread_msg_new(session, &folder_changed_ops, sizeof(*m));
	m->changes = camel_lite_folder_change_info_new();
	camel_lite_folder_change_info_cat(m->changes, changes);
	m->sub = sub;
	camel_lite_object_ref((CamelObject *)sub);
	m->vf = vf;
	camel_lite_object_ref((CamelObject *)vf);
...
{
	camel_lite_vee_folder_remove_folder(vf, f);
}
...
subfolder_renamed_update(CamelVeeFolder *vf, CamelFolder *sub, char hash[8])
{
	...
	count = camel_lite_folder_summary_count(((CamelFolder *)vf)->summary);
	for (i=0;i<count;i++) {
		CamelVeeMessageInfo *mi = (CamelVeeMessageInfo *)camel_lite_folder_summary_index(((CamelFolder *)vf)->summary, i);
		CamelVeeMessageInfo *vinfo;
		...
		if (mi->real->summary == ssummary) {
			...
			camel_lite_folder_change_info_remove_uid(vf->changes, uid);
			camel_lite_folder_summary_remove(((CamelFolder *)vf)->summary, (CamelMessageInfo *)mi);
			...
			if (vinfo) {
				camel_lite_folder_change_info_add_uid(vf->changes, camel_lite_message_info_uid(vinfo));

				/* check unmatched uid's table for any matches */
				if (vf == folder_unmatched
				    && g_hash_table_lookup_extended(unmatched_uids, uid, (void **)&oldkey, &oldval)) {
					g_hash_table_remove(unmatched_uids, oldkey);
					g_hash_table_insert(unmatched_uids, g_strdup(camel_lite_message_info_uid(vinfo)), oldval);
					g_free(oldkey);
				}
			}
		}
	...
	if (changes) {
		camel_lite_object_trigger_event((CamelObject *)vf, "folder_changed", changes);
...
vee_freeze (CamelFolder *folder)
{
	...
	CAMEL_FOLDER_CLASS (camel_lite_vee_folder_parent)->freeze(folder);
}
...
vee_thaw(CamelFolder *folder)
{
	...
	/* call parent implementation */
	CAMEL_FOLDER_CLASS (camel_lite_vee_folder_parent)->thaw(folder);
}
...
vee_set_expression(CamelVeeFolder *vf, const char *query)
{
	...
	while (node) {
		...
		if (camel_lite_vee_folder_rebuild_folder(vf, f, NULL) == -1)
			break;
...
static void
camel_lite_vee_folder_class_init (CamelVeeFolderClass *klass)
{
	CamelFolderClass *folder_class = (CamelFolderClass *) klass;

	camel_lite_vee_folder_parent = CAMEL_FOLDER_CLASS(camel_lite_type_get_global_classfuncs (camel_lite_folder_get_type ()));
...
static void
camel_lite_vee_folder_init (CamelVeeFolder *obj)
{
	...
	obj->changes = camel_lite_folder_change_info_new();
	obj->search = camel_lite_folder_search_new();
...
static void
camel_lite_vee_folder_finalise (CamelObject *obj)
{
	...
	if (vf == folder_unmatched) {
		for (node = p->folders;node;node = g_list_next(node))
			camel_lite_object_unref(node->data);
	} else {
		while (p->folders) {
			CamelFolder *f = p->folders->data;
			camel_lite_vee_folder_remove_folder(vf, f);
	...
	camel_lite_folder_change_info_free(vf->changes);
	camel_lite_object_unref((CamelObject *)vf->search);
