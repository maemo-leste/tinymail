...
static void camel_lite_imapp_store_summary_class_init (CamelIMAPPStoreSummaryClass *klass);
static void camel_lite_imapp_store_summary_init       (CamelIMAPPStoreSummary *obj);
static void camel_lite_imapp_store_summary_finalise   (CamelObject *obj);

static CamelStoreSummaryClass *camel_lite_imapp_store_summary_parent;

static void
camel_lite_imapp_store_summary_class_init (CamelIMAPPStoreSummaryClass *klass)
{
...
static void
camel_lite_imapp_store_summary_init (CamelIMAPPStoreSummary *s)
{
...
static void
camel_lite_imapp_store_summary_finalise (CamelObject *obj)
{
	/*struct _CamelIMAPPStoreSummaryPrivate *p;*/
...
CamelType
camel_lite_imapp_store_summary_get_type (void)
{
	static CamelType type = CAMEL_INVALID_TYPE;

	if (type == CAMEL_INVALID_TYPE) {
		camel_lite_imapp_store_summary_parent = (CamelStoreSummaryClass *)camel_lite_store_summary_get_type();
		type = camel_lite_type_register((CamelType)camel_lite_imapp_store_summary_parent, "CamelLiteIMAPPStoreSummary",
					   sizeof (CamelIMAPPStoreSummary),
					   sizeof (CamelIMAPPStoreSummaryClass),
					   (CamelObjectClassInitFunc) camel_lite_imapp_store_summary_class_init,
					   NULL,
					   (CamelObjectInitFunc) camel_lite_imapp_store_summary_init,
					   (CamelObjectFinalizeFunc) camel_lite_imapp_store_summary_finalise);
	}
...
/**
 * camel_lite_imapp_store_summary_new:
 *
...
camel_lite_imapp_store_summary_new (void)
{
	CamelIMAPPStoreSummary *new = CAMEL_IMAPP_STORE_SUMMARY ( camel_lite_object_new (camel_lite_imapp_store_summary_get_type ()));

	return new;
}

/**
 * camel_lite_imapp_store_summary_full_name:
 * @s:
 * @path:
 *
 ...
 **/
CamelIMAPPStoreInfo *
camel_lite_imapp_store_summary_full_name(CamelIMAPPStoreSummary *s, const char *full_name)
{
	int count, i;
	...
	for (i=0;i<count;i++) {
		info = (CamelIMAPPStoreInfo *)camel_lite_store_summary_index((CamelStoreSummary *)s, i);
		if (info) {
...
char *
camel_lite_imapp_store_summary_full_to_path(CamelIMAPPStoreSummary *s, const char *full_name, char dir_sep)
{
	char *path, *p;
	int c;
	const char *f;
...
char *
camel_lite_imapp_store_summary_path_to_full(CamelIMAPPStoreSummary *s, const char *path, char dir_sep)
{
	...
	if (si && strlen(subpath) == strlen(path)) {
		f = g_strdup(camel_lite_imapp_store_info_full_name(s, si));
		camel_lite_store_summary_info_free((CamelStoreSummary *)s, si);
		return f;
	}

	ns = camel_lite_imapp_store_summary_namespace_find_path(s, path);
	...
	if (si) {
		full = g_strdup_printf("%s%s", camel_lite_imapp_store_info_full_name(s, si), f);
		g_free(f);
		camel_lite_store_summary_info_free((CamelStoreSummary *)s, si);
		f = full;
	} else if (ns) {
...
CamelIMAPPStoreInfo *
camel_lite_imapp_store_summary_add_from_full(CamelIMAPPStoreSummary *s, const char *full, char dir_sep)
{
	CamelIMAPPStoreInfo *info;
	char *pathu8, *prefix;
	int len;
	...
	info = camel_lite_imapp_store_summary_full_name(s, full_name);
	if (info) {
		camel_lite_store_summary_info_free((CamelStoreSummary *)s, (CamelStoreInfo *)info);
	...
	ns = camel_lite_imapp_store_summary_namespace_find_full(s, full_name);
	if (ns) {
		...
		if (len >= strlen(full_name)) {
			...
			prefix = camel_lite_imapp_store_summary_full_to_path(s, full_name+len, ns->sep);
			if (*ns->path) {
		...
		pathu8 = camel_lite_imapp_store_summary_full_to_path(s, full_name, dir_sep);
	}

	info = (CamelIMAPPStoreInfo *)camel_lite_store_summary_add_from_path((CamelStoreSummary *)s, pathu8);
	if (info) {
		d(printf("  '%s' -> '%s'\n", pathu8, full_name));
		camel_lite_store_info_set_string((CamelStoreSummary *)s, (CamelStoreInfo *)info, CAMEL_IMAPP_STORE_INFO_FULL_NAME, full_name);
	} else
		d(printf("  failed\n"));
...
char *
camel_lite_imapp_store_summary_full_from_path(CamelIMAPPStoreSummary *s, const char *path)
{
	CamelIMAPPStoreNamespace *ns;
	char *name = NULL;
	...
	ns = camel_lite_imapp_store_summary_namespace_find_path(s, path);
	if (ns)
		name = camel_lite_imapp_store_summary_path_to_full(s, path, ns->sep);
...
CamelIMAPPStoreNamespace *camel_lite_imapp_store_summary_namespace_new(CamelIMAPPStoreSummary *s, const char *full_name, char dir_sep)
{
	...
	p = ns->path = camel_lite_imapp_store_summary_full_to_path(s, ns->full_name, dir_sep);
	while (*p) {
		if (*p == '/')
			*p = '.';
		p++;
...
camel_lite_imapp_store_summary_namespace_find_path(CamelIMAPPStoreSummary *s, const char *path)
{
	int len;
	CamelIMAPPStoreNamespace *ns;
...
CamelIMAPPStoreNamespace *
camel_lite_imapp_store_summary_namespace_find_full(CamelIMAPPStoreSummary *s, const char *full)
{
	int len;
	CamelIMAPPStoreNamespace *ns;
...
namespace_load(CamelStoreSummary *s, FILE *in)
{
	...
	if (camel_lite_file_util_decode_string(in, &ns->path) == -1
	    || camel_lite_file_util_decode_string(in, &ns->full_name) == -1
	    || camel_lite_file_util_decode_uint32(in, &sep) == -1) {
		namespace_free(s, ns);
		ns = NULL;
	} else {
		ns->sep = sep;
...
namespace_save(CamelStoreSummary *s, FILE *in, CamelIMAPPStoreNamespace *ns)
{
	if (camel_lite_file_util_encode_string(in, ns->path) == -1
	    || camel_lite_file_util_encode_string(in, ns->full_name) == -1
	    || camel_lite_file_util_encode_uint32(in, (guint32)ns->sep) == -1)
		return -1;
...
summary_header_load(CamelStoreSummary *s, FILE *in)
{
	...
	if (camel_lite_imapp_store_summary_parent->summary_header_load((CamelStoreSummary *)s, in) == -1
...
summary_header_save(CamelStoreSummary *s, FILE *out)
{
	...
	if (camel_lite_imapp_store_summary_parent->summary_header_save((CamelStoreSummary *)s, out) == -1
	    || camel_lite_file_util_encode_fixed_int32(out, CAMEL_IMAPP_STORE_SUMMARY_VERSION) == -1
	    || camel_lite_file_util_encode_fixed_int32(out, is->capabilities) == -1
	    || camel_lite_file_util_encode_fixed_int32(out, count) == -1)
		return -1;
...
store_info_load(CamelStoreSummary *s, FILE *in)
{
	...
	mi = (CamelIMAPPStoreInfo *)camel_lite_imapp_store_summary_parent->store_info_load(s, in);
	if (mi) {
...
store_info_save(CamelStoreSummary *s, FILE *out, CamelStoreInfo *mi)
{
	...
	if (camel_lite_imapp_store_summary_parent->store_info_save(s, out, mi) == -1
...
store_info_free(CamelStoreSummary *s, CamelStoreInfo *mi)
{
	...
	camel_lite_imapp_store_summary_parent->store_info_free(s, mi);
}
...
store_info_string(CamelStoreSummary *s, const CamelStoreInfo *mi, int type)
{
	...
	default:
		return camel_lite_imapp_store_summary_parent->store_info_string(s, mi, type);
	}
}
...
store_info_set_string(CamelStoreSummary *s, CamelStoreInfo *mi, int type, const char *str)
{
	...
	case CAMEL_IMAPP_STORE_INFO_FULL_NAME:
		...
		break;
	default:
		camel_lite_imapp_store_summary_parent->store_info_set_string(s, mi, type, str);
		break;
	}
}
