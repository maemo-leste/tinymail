...
static void camel_lite_maildir_folder_class_init(CamelObjectClass * camel_lite_maildir_folder_class)
{
	CamelFolderClass *camel_lite_folder_class = CAMEL_FOLDER_CLASS(camel_lite_maildir_folder_class);
	CamelLocalFolderClass *lclass = (CamelLocalFolderClass *)camel_lite_maildir_folder_class;
	...
	((CamelObjectClass *)camel_lite_folder_class)->getv = maildir_folder_getv;

	camel_lite_folder_class->append_message = maildir_append_message;
	camel_lite_folder_class->get_message = maildir_get_message;
	camel_lite_folder_class->rewrite_cache = maildir_rewrite_cache;
	camel_lite_folder_class->get_allow_external_images = maildir_get_allow_external_images;
	camel_lite_folder_class->set_allow_external_images = maildir_set_allow_external_images;

	//camel_lite_folder_class->transfer_messages_to = maildir_transfer_messages_to;
...
maildir_transfer_messages_to (CamelFolder *source, GPtrArray *uids, CamelFolder *dest, GPtrArray **transferred_uids, gboolean delete_originals, CamelException *ex)
{
	...
	if (delete_originals && CAMEL_IS_MAILDIR_FOLDER (source) && CAMEL_IS_MAILDIR_FOLDER (dest)) {
		...
		camel_lite_operation_start(NULL, _("Moving messages"));
		...
		for (i = 0; i < uids->len; i++) {
			...
			if ((info = camel_lite_folder_summary_uid (source->summary, uid)) == NULL) {
			...
			tmp = camel_lite_maildir_summary_info_to_name (mdi);

			d_filename = g_strdup_printf("%s/cur/%s", df->folder_path, tmp);
			g_free (tmp);
			s_filename = camel_lite_maildir_get_filename (lf->folder_path, mdi, uid);
...
{
	static CamelType camel_lite_maildir_folder_type = CAMEL_INVALID_TYPE;

	if (camel_lite_maildir_folder_type == CAMEL_INVALID_TYPE) {
		camel_lite_maildir_folder_type = camel_lite_type_register(CAMEL_LOCAL_FOLDER_TYPE, "CamelLiteMaildirFolder",
							   sizeof(CamelMaildirFolder),
							   sizeof(CamelMaildirFolderClass),
							   (CamelObjectClassInitFunc) camel_lite_maildir_folder_class_init,
							   NULL,
							   (CamelObjectInitFunc) maildir_init,
							   (CamelObjectFinalizeFunc) maildir_finalize);
	}

	return camel_lite_maildir_folder_type;
}

CamelFolder *
camel_lite_maildir_folder_new(CamelStore *parent_store, const char *full_name, guint32 flags, CamelException *ex)
{
	CamelFolder *folder;

	d(printf("Creating maildir folder: %s\n", full_name));

	folder = (CamelFolder *)camel_lite_object_new(CAMEL_MAILDIR_FOLDER_TYPE);
...
static CamelLocalSummary *maildir_create_summary(CamelLocalFolder *lf, const char *path, const char *folder, CamelIndex *index)
{
	return (CamelLocalSummary *)camel_lite_maildir_summary_new((CamelFolder *)lf, path, folder, index);
}
...
maildir_append_message (CamelFolder *folder, CamelMimeMessage *message, const CamelMessageInfo *info, char **appended_uid, CamelException *ex)
{
	...
	d(printf("Appending message: uid is %s filename is %s\n", camel_lite_message_info_uid(mi), mdi->filename));

	/* write it out to tmp, use the uid we got from the summary */
	name = g_strdup_printf ("%s/tmp/%s", lf->folder_path, camel_lite_message_info_uid(mi));
	...
	dest = camel_lite_maildir_get_filename (lf->folder_path, mdi, mi->uid);
	...
	camel_lite_folder_change_info_clear (((CamelLocalFolder *)maildir_folder)->changes);*/
...
maildir_get_message(CamelFolder * folder, const gchar * uid, CamelFolderReceiveType type, gint param, CamelException * ex)
{
	...
	name = camel_lite_maildir_get_filename (lf->folder_path, mdi, uid);
	...
	if (camel_lite_data_wrapper_construct_from_stream((CamelDataWrapper *)message, message_stream) == -1) {
		camel_lite_exception_setv(ex, (errno==EINTR)?CAMEL_EXCEPTION_USER_CANCEL:CAMEL_EXCEPTION_SYSTEM_IO_READ,
				     _("Cannot get message: %s from folder %s\n  %s"),
...
maildir_rewrite_cache (CamelFolder *folder, const char *uid, CamelMimeMessage *msg)
{
	...
	if (camel_lite_data_wrapper_write_to_stream ((CamelDataWrapper *)msg, output_stream) == -1
	    || camel_lite_stream_close (output_stream) == -1)
		goto fail_write;

	/* now move from tmp to cur (bypass new, does it matter?) */
	dest = camel_lite_maildir_get_filename (lf->folder_path, mdi, info->uid);
	if (rename (name, dest) == -1)
		goto fail_write;
