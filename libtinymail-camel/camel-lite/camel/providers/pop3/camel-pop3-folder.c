...
pop3_finalize (CamelObject *object)
{
	...
	camel_lite_folder_summary_save (folder->summary, NULL);

	return;
}

static void
camel_lite_pop3_summary_set_extra_flags (CamelFolder *folder, CamelMessageInfoBase *mi)
{
	CamelPOP3Store *pop3_store = (CamelPOP3Store *)folder->parent_store;
	camel_lite_data_cache_set_flags (pop3_store->cache, "cache", mi);
}

CamelFolder *
camel_lite_pop3_folder_new (CamelStore *parent, CamelException *ex)
{
	CamelFolder *folder;
	...
	folder = CAMEL_FOLDER (camel_lite_object_new (CAMEL_POP3_FOLDER_TYPE));

	camel_lite_folder_construct (folder, parent, "inbox", "inbox");

	check_dir (p3store, NULL);

	summary_file = g_strdup_printf ("%s/summary.mmap", p3store->storage_path);
	folder->summary = camel_lite_folder_summary_new (folder);
	if (folder->summary)
		folder->summary->set_extra_flags_func = camel_lite_pop3_summary_set_extra_flags;

	camel_lite_folder_summary_set_build_content (folder->summary, FALSE);
	camel_lite_folder_summary_set_filename (folder->summary, summary_file);

	if (camel_lite_folder_summary_load (folder->summary) == -1) {
...
pop3_get_local_size (CamelFolder *folder)
{
	...
	int msize = 0;
	check_dir (p3store, NULL);
	camel_lite_du (p3store->storage_path , &msize);
	return msize;
}
...
cmd_builduid(CamelPOP3Engine *pe, CamelPOP3Stream *stream, void *data)
{
	...
	camel_lite_operation_progress_count(NULL, fi->id);

	md5_init(&md5);
	mp = camel_lite_mime_parser_new();
	camel_lite_mime_parser_init_with_stream(mp, (CamelStream *)stream);
	switch (camel_lite_mime_parser_step(mp, NULL, NULL)) {
	case CAMEL_MIME_PARSER_STATE_HEADER:
	case CAMEL_MIME_PARSER_STATE_MESSAGE:
	case CAMEL_MIME_PARSER_STATE_MULTIPART:
		h = camel_lite_mime_parser_headers_raw(mp);
		while (h) {
		...
		if (ret>=0) {
			if (sscanf((char *) line, "%u %u", &id, &size) == 2) {
				...
				if ((pop3_store->engine && pop3_store->engine->capa & CAMEL_POP3_CAP_UIDL) == 0)
					fi->cmd = camel_lite_pop3_engine_command_new(pe, CAMEL_POP3_COMMAND_MULTI, cmd_builduid, fi, "TOP %u 0\r\n", id);
	...
	do {
		...
		if (ret>=0) {
			...
			if (sscanf((char *) line, "%u %s", &id, uid) == 2) {
				...
				if (fi) {
					camel_lite_operation_progress(NULL, (fi->index+1) , pop3_store->uids->len);
					fi->uid = g_strdup(uid);
					pop3_debug ("%s added\n", fi->uid);
...
pop3_refresh_info (CamelFolder *folder, CamelException *ex)
{
	...
	if (camel_lite_disco_store_status (CAMEL_DISCO_STORE (pop3_store)) == CAMEL_DISCO_STORE_OFFLINE)
		return;
	...
	{
		camel_lite_service_connect (CAMEL_SERVICE (pop3_store), ex);
		if (camel_lite_exception_is_set (ex)) {
	...
	pcl = camel_lite_pop3_engine_command_new(pop3_store->engine, CAMEL_POP3_COMMAND_MULTI, cmd_list, folder, "LIST\r\n");
	if (pop3_store->engine->capa & CAMEL_POP3_CAP_UIDL)
		pcu = camel_lite_pop3_engine_command_new(pop3_store->engine, CAMEL_POP3_COMMAND_MULTI, cmd_uidl, folder, "UIDL\r\n");
	while ((i = camel_lite_pop3_engine_iterate(pop3_store->engine, NULL)) > 0)
		;
	...
	if (i == -1) {
		if (errno == EINTR)
			...
			camel_lite_exception_setv (ex, CAMEL_EXCEPTION_SERVICE_PROTOCOL,
	...
	camel_lite_pop3_engine_command_free(pop3_store->engine, pcl);
	...
	camel_lite_pop3_logbook_open (pop3_store->book);
	...
	for (i=0;i<pop3_store->uids->len;i++) {
		...
		mi = (CamelMessageInfoBase*) camel_lite_folder_summary_uid (folder->summary, fi->uid);

		if (!mi && !camel_lite_pop3_logbook_is_registered (pop3_store->book, fi->uid)) {
			...
			if (msg)
			{
				...
				camel_lite_folder_change_info_add_uid (changes, fi->uid);
				...
				camel_lite_pop3_logbook_register (pop3_store->book, fi->uid);
		...
		camel_lite_operation_progress (NULL, i , pop3_store->uids->len);
	...
	{
		if (pop3_store->engine->capa & CAMEL_POP3_CAP_UIDL) {
			camel_lite_pop3_engine_command_free(pop3_store->engine, pcu);
		} else {
			for (i=0;i<pop3_store->uids->len;i++) {
				CamelPOP3FolderInfo *fi = pop3_store->uids->pdata[i];

				if (fi->cmd) {
					camel_lite_pop3_engine_command_free(pop3_store->engine, fi->cmd);
					fi->cmd = NULL;
				}
				if (fi->uid)
					g_hash_table_insert(pop3_store->uids_uid, fi->uid, fi);
	...
	for (i = 0; i < max; i++) {
		...
		info = (CamelMessageInfoBase*) camel_lite_folder_summary_index (folder->summary, i);
		if (!info)
			continue;
		...
		if (!found) {
			...
			if (info && info->uid && !camel_lite_data_cache_exists (pop3_store->cache, "cache", info->uid, NULL)) {
	...
	while (deleted)
	{
		...
		if (!changes)
			changes = camel_lite_folder_change_info_new ();
		camel_lite_folder_change_info_remove_uid (changes, info->uid);
	...
	if (changes && camel_lite_folder_change_info_changed (changes)) {
		camel_lite_object_trigger_event (CAMEL_OBJECT (folder), "folder_changed", changes);
		camel_lite_folder_change_info_free (changes);
		changes = NULL;
	}
	...
	camel_lite_pop3_logbook_close (pop3_store->book);
...
pop3_sync (CamelFolder *folder, gboolean expunge, CamelException *ex)
{
	...
	{
		camel_lite_operation_start(NULL, _("Expunging old messages"));
		camel_lite_pop3_delete_old(folder, pop3_store->delete_after,ex);
		camel_lite_operation_end (NULL);
	}
	...
	camel_lite_operation_start(NULL, _("Expunging deleted messages"));
	...
	for (i = 0; i < max; i++) {
		...
		if (info->flags & CAMEL_MESSAGE_DELETED)
		{
			...
			if (pop3_store->cache && info->uid)
				camel_lite_data_cache_remove(pop3_store->cache, "cache", info->uid, NULL);
			...
			if (expunge) {
				...
				if (fi) {
					cmd = camel_lite_pop3_engine_command_new(pop3_store->engine,
									    0, NULL, NULL, "DELE %u\r\n",
									    fi->id);
					while (camel_lite_pop3_engine_iterate(pop3_store->engine, cmd) > 0);
					camel_lite_pop3_engine_command_free(pop3_store->engine, cmd);
				}
			}
			g_static_rec_mutex_unlock (pop3_store->eng_lock);
		}
	...
	g_static_rec_mutex_lock (pop3_store->eng_lock);
	camel_lite_pop3_store_expunge (pop3_store, ex);
	g_static_rec_mutex_unlock (pop3_store->eng_lock);
...
pop3_get_message_time_from_cache (CamelFolder *folder, const char *uid, time_t *message_time)
{
	...
	if ((stream = camel_lite_data_cache_get (pop3_store->cache, "cache", uid, NULL)) != NULL
	    && camel_lite_stream_read (stream, buffer, 1) == 1
	    && buffer[0] == '#') {
...
int
camel_lite_pop3_delete_old(CamelFolder *folder, int days_to_delete,	CamelException *ex)
{
	...
	for (i = 0; i < pop3_store->uids->len; i++) {
		...
		if (pop3_get_message_time_from_cache (folder, fi->uid, &message_time)) {
			...
			{
				if (fi->cmd) {
					while (camel_lite_pop3_engine_iterate(pop3_store->engine, fi->cmd) > 0) {
				...
				fi->cmd = camel_lite_pop3_engine_command_new(pop3_store->engine,
									0,
									NULL,
									NULL,
									"DELE %u\r\n",
				...
				if (pop3_store->cache && fi->uid) {
					camel_lite_data_cache_remove(pop3_store->cache, "cache", fi->uid, NULL);
				}
			}
		}
	}
	...
	for (i = 0; i < pop3_store->uids->len; i++) {
		...
		camel_lite_operation_progress(NULL, (i+1) , pop3_store->uids->len);
	}
...
cmd_tocache(CamelPOP3Engine *pe, CamelPOP3Stream *stream, void *data)
{
	...
	if ((n = camel_lite_stream_write(fi->stream, "*", 1)) == -1)
		goto done;

	while ((n = camel_lite_stream_read((CamelStream *)stream, buffer, sizeof(buffer) - 1)) > 0) {
		n = camel_lite_stream_write(fi->stream, buffer, n);
		if (n == -1)
			break;

		buffer[n] = '\0';
		/* heuristics */
		if (camel_lite_strstrcase (buffer, "Content-Disposition: attachment") != NULL)
			fi->has_attachments = TRUE;
		else if (camel_lite_strstrcase (buffer, "filename=") != NULL &&
			 strchr (buffer, '.'))
			fi->has_attachments = TRUE;
		else if (camel_lite_strstrcase (buffer, "Content-Type: message/rfc822") != NULL)
			fi->has_attachments = TRUE;
		...
		if (w > fi->size)
			w = fi->size;
		if (fi->size != 0)
			camel_lite_operation_progress(NULL, w , fi->size);
	}

	/* it all worked, output a '#' to say we're a-ok */
	if (n != -1) {
		camel_lite_stream_reset(fi->stream);
		n = camel_lite_stream_write(fi->stream, "#", 1);

		camel_lite_data_cache_set_partial (tstore->cache, "cache", fi->uid, FALSE);
	}
done:
	if (n == -1) {
		fi->err = errno;
	...
	camel_lite_object_unref((CamelObject *)fi->stream);
	fi->stream = NULL;
...
cmd_tocache_partial (CamelPOP3Engine *pe, CamelPOP3Stream *stream, void *data)
{
	...
	while (!theend && camel_lite_pop3_stream_line (stream, &buffer, &len) > 0)
	{
		if (!buffer)
			continue;

		/* heuristics */
		if (buffer && camel_lite_strstrcase ((const char *) buffer, "Content-Disposition: attachment") != NULL)
			fi->has_attachments = TRUE;
		else if (buffer && camel_lite_strstrcase ((const char *) buffer, "filename=") != NULL &&
			 strchr ((const char *) buffer, '.'))
			fi->has_attachments = TRUE;
		else if (buffer && camel_lite_strstrcase ((const char *) buffer, "Content-Type: message/rfc822") != NULL)
			fi->has_attachments = TRUE;
		...
		if (boundary == NULL && buffer)
		{
			   ...
			   char *pstr = (char*)camel_lite_strstrcase ((const char *) buffer, "Content-Type:");
		...
		} else if (buffer && strstr ((const char*) buffer, (const char*) boundary))
		{
			if (occurred)
			{
				...
				camel_lite_service_disconnect (CAMEL_SERVICE (tstore), FALSE, &myex);
				camel_lite_service_connect (CAMEL_SERVICE (tstore), &myex);
		...
		if (!theend && buffer)
		{
		    n = camel_lite_stream_write(fi->stream, (const char*) buffer, len);
		    if (n == -1 || camel_lite_stream_write(fi->stream, "\n", 1) == -1)
			break;
		    w += n+1;
		} else if (boundary != NULL)
		{
		    gchar *nb = g_strdup_printf ("\n--%s\n", boundary);
		    n = camel_lite_stream_write(fi->stream, nb, strlen (nb));
		    g_free (nb);
		}
	...
	if (n != -1 || theend) {
		...
		n = camel_lite_stream_write(fi->stream, "#", 1);
		if (theend)
			camel_lite_data_cache_set_partial (tstore->cache, "cache", fi->uid, TRUE);
		else
 			camel_lite_data_cache_set_partial (tstore->cache, "cache", fi->uid, FALSE);
...
{
	CamelPOP3Store *pop3_store = CAMEL_POP3_STORE (folder->parent_store);
	camel_lite_data_cache_delete_attachments (pop3_store->cache, "cache", uid);
	return;
}
...
pop3_get_allow_external_images (CamelFolder *folder, const char *uid)
{
	gboolean retval;
	CamelPOP3Store *pop3_store = CAMEL_POP3_STORE (folder->parent_store);
	retval = camel_lite_data_cache_get_allow_external_images (pop3_store->cache, "cache", uid);
	return retval;
}
...
pop3_set_allow_external_images (CamelFolder *folder, const char *uid, gboolean allow)
{
	gboolean retval;
	CamelPOP3Store *pop3_store = CAMEL_POP3_STORE (folder->parent_store);
	camel_lite_data_cache_set_allow_external_images (pop3_store->cache, "cache", uid, allow);
	return;
}
...
pop3_get_message (CamelFolder *folder, const char *uid, CamelFolderReceiveType type, gint param, CamelException *ex)
{
	...
	stream = camel_lite_data_cache_get (pop3_store->cache, "cache", uid, NULL);
	if (stream)
	{
		message = camel_lite_mime_message_new ();
		if (camel_lite_data_wrapper_construct_from_stream((CamelDataWrapper *)message, stream) == -1) {
			if (errno == EINTR)
				camel_lite_exception_setv(ex, CAMEL_EXCEPTION_USER_CANCEL, "User canceled");
			else
	...
	if (fi == NULL) 
	{
		...
		if (!pop3_store->is_refreshing) 
		{
			...
			camel_lite_pop3_store_destroy_lists (pop3_store);
			pcl = camel_lite_pop3_engine_command_new(pop3_store->engine, CAMEL_POP3_COMMAND_MULTI, cmd_list, folder, "LIST\r\n");
...
rfail:
		if (fi == NULL) {
			...
			if (camel_lite_operation_cancel_check (NULL)) {
				camel_lite_exception_setv(ex, CAMEL_EXCEPTION_USER_CANCEL,
					"User canceled message retrieval");
			} else {
				CamelFolder *folder = (CamelFolder *) pop3_folder;
				CamelMessageInfo *mi = camel_lite_folder_summary_uid (folder->summary, uid);
				if (mi) {
					CamelFolderChangeInfo *changes = camel_lite_folder_change_info_new ();
					((CamelMessageInfoBase*)mi)->flags |= CAMEL_MESSAGE_EXPUNGED;
					if (mi->uid) {
						camel_lite_folder_change_info_remove_uid (changes, mi->uid);
						camel_lite_object_trigger_event (CAMEL_OBJECT (folder), "folder_changed", changes);
					}
					camel_lite_folder_summary_remove (folder->summary, mi);
					camel_lite_folder_change_info_free (changes);
					camel_lite_message_info_free (mi);
				}
   ...
   while (retry < 2)
   {
	...
	if (fi->cmd != NULL) {
		...
		while ((i = camel_lite_pop3_engine_iterate(pop3_store->engine, fi->cmd)) > 0)
			;
	...
	if (pop3_store->cache != NULL)
	{
		...
		if ((type & CAMEL_FOLDER_RECEIVE_FULL) && camel_lite_data_cache_is_partial (pop3_store->cache, "cache", fi->uid))
		{

			camel_lite_data_cache_remove (pop3_store->cache, "cache", fi->uid, &tex);
			im_certain = TRUE;

		} else if ((type & CAMEL_FOLDER_RECEIVE_PARTIAL)
			&& !camel_lite_data_cache_is_partial (pop3_store->cache, "cache", fi->uid))
		{
			camel_lite_data_cache_remove (pop3_store->cache, "cache", fi->uid, &tex);
			im_certain = TRUE;
		}
	...
	if (im_certain || pop3_store->cache == NULL
	    || (stream = camel_lite_data_cache_get(pop3_store->cache, "cache", fi->uid, NULL)) == NULL
	    || camel_lite_stream_read(stream, buffer, 1) != 1
	    || buffer[0] != '#') {

		/* Initiate retrieval, if disk backing fails, use a memory backing */
		if (pop3_store->cache == NULL
		    || (stream = camel_lite_data_cache_add(pop3_store->cache, "cache", fi->uid, ex)) == NULL) {
		...
		if (type & CAMEL_FOLDER_RECEIVE_FULL || type & CAMEL_FOLDER_RECEIVE_ANY_OR_FULL) {
			...
			pcr = camel_lite_pop3_engine_command_new(pop3_store->engine, CAMEL_POP3_COMMAND_MULTI,
		...
		while ((i = camel_lite_pop3_engine_iterate(pop3_store->engine, pcr)) > 0)
			;
		if (i == -1)
			fi->err = errno;
		...
		camel_lite_pop3_engine_command_free(pop3_store->engine, pcr);
		...
		if (fi->err != 0) {
			...
			camel_lite_data_cache_remove (pop3_store->cache, "cache", fi->uid, &iex);
			camel_lite_exception_clear (&iex);
			message = NULL;

			goto done;
		}

		if (camel_lite_stream_read(stream, buffer, 1) != 1 || buffer[0] != '#') {
			camel_lite_exception_setv (ex, CAMEL_EXCEPTION_SERVICE_PROTOCOL,
	...
	if (camel_lite_data_wrapper_construct_from_stream((CamelDataWrapper *)message, stream) == -1) {
		...
		if (type & CAMEL_FOLDER_RECEIVE_FULL && pop3_store->immediate_delete_after)
		{
			...
			cmd = camel_lite_pop3_engine_command_new(pop3_store->engine, 0, NULL, NULL, "DELE %u\r\n", uid);
			while (camel_lite_pop3_engine_iterate(pop3_store->engine, cmd) > 0);
			camel_lite_pop3_engine_command_free(pop3_store->engine, cmd);
	...
	mi = (CamelMessageInfoBase *) camel_lite_folder_summary_uid (summary, uid);
	if (!mi && !camel_lite_pop3_logbook_is_registered (pop3_store->book, uid) && message) {
		mi = (CamelMessageInfoBase *) camel_lite_folder_summary_info_new_from_message (summary, message);
		if (mi->uid)
			g_free (mi->uid);
		mi->uid = g_strdup (uid);
		if (had_attachment)
			camel_lite_message_info_set_flags((CamelMessageInfo *) mi, CAMEL_MESSAGE_ATTACHMENTS, CAMEL_MESSAGE_ATTACHMENTS);
...
pop3_get_top (CamelFolder *folder, const char *uid, CamelException *ex)
{
	...
	/*camel_lite_operation_start_transient(NULL, "Retrieving summary info for %d", fi->id);*/
	...
	if (fi->cmd != NULL) {
		...
		camel_lite_object_ref (mengine);

		g_static_rec_mutex_unlock (pop3_store->eng_lock);

		while ((i = camel_lite_pop3_engine_iterate(mengine, fi->cmd)) > 0)
			;
		...
		camel_lite_object_unref (mengine);
...
{
	camel_lite_exception_set (ex, CAMEL_EXCEPTION_SERVICE_NOT_SUPPORTED, _("Not supported"));
...
static void
camel_lite_pop3_folder_class_init (CamelPOP3FolderClass *camel_lite_pop3_folder_class)
{
	CamelFolderClass *camel_lite_folder_class = CAMEL_FOLDER_CLASS(camel_lite_pop3_folder_class);
	CamelDiscoFolderClass *camel_lite_disco_folder_class = CAMEL_DISCO_FOLDER_CLASS (camel_lite_pop3_folder_class);
	...
	camel_lite_folder_class->get_local_size = pop3_get_local_size;
	camel_lite_folder_class->refresh_info = pop3_refresh_info;
	camel_lite_folder_class->sync = pop3_sync;

	camel_lite_folder_class->get_uids = pop3_get_uids;
	camel_lite_folder_class->free_uids = camel_lite_folder_free_shallow;
	camel_lite_folder_class->get_message = pop3_get_message;
	camel_lite_folder_class->set_message_flags = pop3_set_message_flags;
	camel_lite_folder_class->delete_attachments = pop3_delete_attachments;
	camel_lite_folder_class->get_allow_external_images = pop3_get_allow_external_images;
	camel_lite_folder_class->set_allow_external_images = pop3_set_allow_external_images;

	camel_lite_disco_folder_class->refresh_info_online = pop3_refresh_info;
	camel_lite_disco_folder_class->sync_online = pop3_sync_online;
	camel_lite_disco_folder_class->sync_offline = pop3_sync_offline;

	camel_lite_disco_folder_class->sync_resyncing = pop3_sync_offline;

	camel_lite_disco_folder_class->expunge_uids_online = pop3_expunge_uids_online;
	camel_lite_disco_folder_class->expunge_uids_offline = pop3_expunge_uids_offline;
	camel_lite_disco_folder_class->expunge_uids_resyncing = pop3_expunge_uids_resyncing;

	camel_lite_disco_folder_class->append_online = pop3_append_offline;
	camel_lite_disco_folder_class->append_offline = pop3_append_offline;
	camel_lite_disco_folder_class->append_resyncing = pop3_append_offline;

	camel_lite_disco_folder_class->transfer_online = pop3_transfer_offline;
	camel_lite_disco_folder_class->transfer_offline = pop3_transfer_offline;
	camel_lite_disco_folder_class->transfer_resyncing = pop3_transfer_offline;

	camel_lite_disco_folder_class->cache_message = pop3_cache_message;
}

CamelType
camel_lite_pop3_folder_get_type (void)
{
	static CamelType camel_lite_pop3_folder_type = CAMEL_INVALID_TYPE;

	if (!camel_lite_pop3_folder_type) {
		camel_lite_pop3_folder_type = camel_lite_type_register (CAMEL_DISCO_FOLDER_TYPE, "CamelLitePOP3Folder",
							      sizeof (CamelPOP3Folder),
							      sizeof (CamelPOP3FolderClass),
							      (CamelObjectClassInitFunc) camel_lite_pop3_folder_class_init,
							      NULL,
							      NULL,
							      (CamelObjectFinalizeFunc) pop3_finalize);
	}

	return camel_lite_pop3_folder_type;
}
