...
static void camel_lite_imap4_journal_class_init (CamelIMAP4JournalClass *klass);
static void camel_lite_imap4_journal_init (CamelIMAP4Journal *journal, CamelIMAP4JournalClass *klass);
static void camel_lite_imap4_journal_finalize (CamelObject *object);
...
CamelType
camel_lite_imap4_journal_get_type (void)
{
	static CamelType type = 0;

	if (!type) {
		type = camel_lite_type_register (camel_lite_offline_journal_get_type (),
					    "CamelLiteIMAP4Journal",
					    sizeof (CamelIMAP4Journal),
					    sizeof (CamelIMAP4JournalClass),
					    (CamelObjectClassInitFunc) camel_lite_imap4_journal_class_init,
					    NULL,
					    (CamelObjectInitFunc) camel_lite_imap4_journal_init,
					    (CamelObjectFinalizeFunc) camel_lite_imap4_journal_finalize);
	}
...
static void
camel_lite_imap4_journal_class_init (CamelIMAP4JournalClass *klass)
{
	CamelOfflineJournalClass *journal_class = (CamelOfflineJournalClass *) klass;

	parent_class = (CamelOfflineJournalClass *) camel_lite_type_get_global_classfuncs (CAMEL_TYPE_OFFLINE_JOURNAL);
...
static void
camel_lite_imap4_journal_init (CamelIMAP4Journal *journal, CamelIMAP4JournalClass *klass)
{
	journal->failed = g_ptr_array_new ();
}

static void
camel_lite_imap4_journal_finalize (CamelObject *object)
{
	...
	if (journal->failed) {
		for (i = 0; i < journal->failed->len; i++)
			camel_lite_message_info_free (journal->failed->pdata[i]);
...
imap4_entry_load (CamelOfflineJournal *journal, FILE *in)
{
	...
	if (camel_lite_file_util_decode_uint32 (in, &entry->type) == -1)
		goto exception;

	switch (entry->type) {
	case CAMEL_IMAP4_JOURNAL_ENTRY_APPEND:
		if (camel_lite_file_util_decode_string (in, &entry->v.append_uid) == -1)
			goto exception;
...
imap4_entry_write (CamelOfflineJournal *journal, EDListNode *entry, FILE *out)
{
	...
	if (camel_lite_file_util_encode_uint32 (out, imap4_entry->type) == -1)
		return -1;

	switch (imap4_entry->type) {
	case CAMEL_IMAP4_JOURNAL_ENTRY_APPEND:
		if (camel_lite_file_util_encode_string (out, imap4_entry->v.append_uid))
			return -1;
...
{
	camel_lite_flag_list_copy (&dest->user_flags, &src->user_flags);
	camel_lite_tag_list_copy (&dest->user_tags, &src->user_tags);
...
imap4_entry_play_append (CamelOfflineJournal *journal, CamelIMAP4JournalEntry *entry, CamelException *ex)
{
	...
	if (!imap4_folder->cache || !(stream = camel_lite_data_cache_get (imap4_folder->cache, "cache", entry->v.append_uid, ex)))
		goto done;
	...
	if (!(info = camel_lite_folder_summary_uid (folder->summary, entry->v.append_uid))) {
		...
		info = camel_lite_message_info_new (NULL);
	}

	camel_lite_exception_init (&lex);
	camel_lite_folder_append_message (folder, message, info, &uid, &lex);
	camel_lite_object_unref (message);
	...
	if (camel_lite_exception_is_set (&lex)) {
		...
		if (info->summary == folder->summary) {
			...
			camel_lite_folder_summary_remove_uid (folder->summary, entry->v.append_uid);
	...
	if (uid != NULL && (real = camel_lite_folder_summary_uid (folder->summary, uid))) {
	...
	camel_lite_folder_summary_remove_uid (folder->summary, entry->v.append_uid);
	camel_lite_data_cache_remove (imap4_folder->cache, "cache", entry->v.append_uid, NULL);
...
CamelOfflineJournal *
camel_lite_imap4_journal_new (CamelIMAP4Folder *folder, const char *filename)
{
	CamelOfflineJournal *journal;

	g_return_val_if_fail (CAMEL_IS_IMAP4_FOLDER (folder), NULL);

	journal = (CamelOfflineJournal *) camel_lite_object_new (camel_lite_imap4_journal_get_type ());
	camel_lite_offline_journal_construct (journal, (CamelFolder *) folder, filename);
...
camel_lite_imap4_journal_readd_failed (CamelIMAP4Journal *journal)
{
	...
	for (i = 0; i < journal->failed->len; i++)
		camel_lite_folder_summary_add (summary, journal->failed->pdata[i]);
...
void
camel_lite_imap4_journal_append (CamelIMAP4Journal *imap4_journal, CamelMimeMessage *message,
			    const CamelMessageInfo *mi, char **appended_uid, CamelException *ex)
{
	...
	nextuid = camel_lite_folder_summary_next_uid (folder->summary);
	uid = g_strdup_printf ("-%lu", nextuid);

	if (!(cache = camel_lite_data_cache_add (imap4_folder->cache, "cache", uid, ex))) {
		folder->summary->nextuid--;
		g_free (uid);
		return;
	}

	if (camel_lite_data_wrapper_write_to_stream ((CamelDataWrapper *) message, cache) == -1
	...
	info = camel_lite_folder_summary_info_new_from_message (folder->summary, message);
	g_free(info->uid);
	info->uid = g_strdup (uid);

	imap4_message_info_dup_to ((CamelMessageInfoBase *) info, (CamelMessageInfoBase *) mi);

	camel_lite_folder_summary_add (folder->summary, info);
