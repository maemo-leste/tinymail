 ...
 **/
CamelCipherContext *
camel_lite_cipher_context_new (CamelSession *session)
{
	CamelCipherContext *context;

	g_return_val_if_fail (session != NULL, NULL);

	context = CAMEL_CIPHER_CONTEXT (camel_lite_object_new (CAMEL_CIPHER_CONTEXT_TYPE));
...
/**
 * camel_lite_cipher_context_construct:
 ...
 **/
void
camel_lite_cipher_context_construct (CamelCipherContext *context, CamelSession *session)
{
 ...
 **/
int
camel_lite_cipher_sign (CamelCipherContext *context, const char *userid, CamelCipherHash hash,
		   struct _CamelMimePart *ipart, struct _CamelMimePart *opart, CamelException *ex)
{
	int retval;
	...
	camel_lite_operation_start(NULL, _("Signing message"));
 ...
 **/
CamelCipherValidity *
camel_lite_cipher_verify (CamelCipherContext *context, struct _CamelMimePart *ipart, CamelException *ex)
{
	CamelCipherValidity *valid;

	g_return_val_if_fail (CAMEL_IS_CIPHER_CONTEXT (context), NULL);

	camel_lite_operation_start(NULL, _("Verifying message"));
 ...
 **/
int
camel_lite_cipher_encrypt (CamelCipherContext *context, const char *userid, GPtrArray *recipients,
		      struct _CamelMimePart *ipart, struct _CamelMimePart *opart, CamelException *ex)
{
	int retval;
	...
	camel_lite_operation_start(NULL, _("Encrypting message"));
 ...
 **/
CamelCipherValidity *
camel_lite_cipher_decrypt(CamelCipherContext *context, struct _CamelMimePart *ipart, struct _CamelMimePart *opart, CamelException *ex)
{
	CamelCipherValidity *valid;

	g_return_val_if_fail (CAMEL_IS_CIPHER_CONTEXT (context), NULL);

	camel_lite_operation_start(NULL, _("Decrypting message"));
 ...
 **/
int
camel_lite_cipher_import_keys (CamelCipherContext *context, struct _CamelStream *istream, CamelException *ex)
{
	g_return_val_if_fail (CAMEL_IS_CIPHER_CONTEXT (context), -1);
 ...
 **/
int
camel_lite_cipher_export_keys (CamelCipherContext *context, GPtrArray *keys,
			  struct _CamelStream *ostream, CamelException *ex)
{
	g_return_val_if_fail (CAMEL_IS_CIPHER_CONTEXT (context), -1);
...
CamelCipherHash
camel_lite_cipher_id_to_hash(CamelCipherContext *context, const char *id)
{
...
const char *
camel_lite_cipher_hash_to_id(CamelCipherContext *context, CamelCipherHash hash)
{
	g_return_val_if_fail (CAMEL_IS_CIPHER_CONTEXT (context), NULL);
...
CamelCipherValidity *
camel_lite_cipher_validity_new (void)
{
	CamelCipherValidity *validity;

	validity = g_malloc(sizeof(*validity));
	camel_lite_cipher_validity_init(validity);

	return validity;
}

void
camel_lite_cipher_validity_init (CamelCipherValidity *validity)
{
...
gboolean
camel_lite_cipher_validity_get_valid (CamelCipherValidity *validity)
{
...
void
camel_lite_cipher_validity_set_valid (CamelCipherValidity *validity, gboolean valid)
{
	g_assert (validity != NULL);
...
gchar *
camel_lite_cipher_validity_get_description (CamelCipherValidity *validity)
{
	if (validity == NULL)
		return NULL;
...
void
camel_lite_cipher_validity_set_description (CamelCipherValidity *validity, const gchar *description)
{
	g_assert (validity != NULL);
...
void
camel_lite_cipher_validity_clear (CamelCipherValidity *validity)
{
	g_assert (validity != NULL);
...
CamelCipherValidity *
camel_lite_cipher_validity_clone(CamelCipherValidity *vin)
{
	CamelCipherValidity *vo;
	CamelCipherCertInfo *info;

	vo = camel_lite_cipher_validity_new();
	...
	while (info->next) {
		camel_lite_cipher_validity_add_certinfo(vo, CAMEL_CIPHER_VALIDITY_SIGN, info->name, info->email);
		info = info->next;
	}

	info = (CamelCipherCertInfo *)vin->encrypt.encrypters.head;
	while (info->next) {
		camel_lite_cipher_validity_add_certinfo(vo, CAMEL_CIPHER_VALIDITY_ENCRYPT, info->name, info->email);
		info = info->next;
	}
...
/**
 * camel_lite_cipher_validity_add_certinfo:
 * @vin:
 * @mode:
 * @name:
 * @email:
 ...
 **/
void
camel_lite_cipher_validity_add_certinfo(CamelCipherValidity *vin, enum _camel_lite_cipher_validity_mode_t mode, const char *name, const char *email)
{
	CamelCipherCertInfo *info;
	EDList *list;
...
/**
 * camel_lite_cipher_validity_envelope:
 * @parent:
 * @valid:
 *
 ...
 **/
void
camel_lite_cipher_validity_envelope(CamelCipherValidity *parent, CamelCipherValidity *valid)
{
	CamelCipherCertInfo *info;
	...
	if (parent->sign.status != CAMEL_CIPHER_VALIDITY_SIGN_NONE
		...
		while (info->next) {
			camel_lite_cipher_validity_add_certinfo(parent, CAMEL_CIPHER_VALIDITY_ENCRYPT, info->name, info->email);
			info = info->next;
		}
		...
		while (info->next) {
			camel_lite_cipher_validity_add_certinfo(parent, CAMEL_CIPHER_VALIDITY_SIGN, info->name, info->email);
			info = info->next;
		}
	}
...
void
camel_lite_cipher_validity_free (CamelCipherValidity *validity)
{
	...
	while ((child = (CamelCipherValidity *)e_dlist_remhead(&validity->children)))
		camel_lite_cipher_validity_free(child);
	...
	camel_lite_cipher_validity_clear(validity);
	g_free(validity);
}
...
static void
camel_lite_cipher_context_init (CamelCipherContext *context)
{
...
static void
camel_lite_cipher_context_finalise (CamelObject *o)
{
	CamelCipherContext *context = (CamelCipherContext *)o;

	camel_lite_object_unref (CAMEL_OBJECT (context->session));
...
static void
camel_lite_cipher_context_class_init (CamelCipherContextClass *camel_lite_cipher_context_class)
{
	parent_class = camel_lite_type_get_global_classfuncs (camel_lite_object_get_type ());

	camel_lite_cipher_context_class->hash_to_id = cipher_hash_to_id;
	camel_lite_cipher_context_class->id_to_hash = cipher_id_to_hash;
	camel_lite_cipher_context_class->sign = cipher_sign;
	camel_lite_cipher_context_class->verify = cipher_verify;
	camel_lite_cipher_context_class->encrypt = cipher_encrypt;
	camel_lite_cipher_context_class->decrypt = cipher_decrypt;
	camel_lite_cipher_context_class->import_keys = cipher_import_keys;
	camel_lite_cipher_context_class->export_keys = cipher_export_keys;
}

CamelType
camel_lite_cipher_context_get_type (void)
{
	static CamelType type = CAMEL_INVALID_TYPE;

	if (type == CAMEL_INVALID_TYPE) {
		type = camel_lite_type_register (camel_lite_object_get_type (),
					    "CamelLiteCipherContext",
					    sizeof (CamelCipherContext),
					    sizeof (CamelCipherContextClass),
					    (CamelObjectClassInitFunc) camel_lite_cipher_context_class_init,
					    NULL,
					    (CamelObjectInitFunc) camel_lite_cipher_context_init,
					    (CamelObjectFinalizeFunc) camel_lite_cipher_context_finalise);
	}
...
cc_prepare_sign(CamelMimePart *part)
{
	...
	dw = camel_lite_medium_get_content_object((CamelMedium *)part);
	if (!dw)
		return;

	if (CAMEL_IS_MULTIPART (dw)) {
		parts = camel_lite_multipart_get_number((CamelMultipart *)dw);
		for (i = 0; i < parts; i++)
			cc_prepare_sign(camel_lite_multipart_get_part((CamelMultipart *)dw, i));
		...
		if (encoding != CAMEL_TRANSFER_ENCODING_BASE64
			...
			camel_lite_mime_part_set_encoding(part, CAMEL_TRANSFER_ENCODING_QUOTEDPRINTABLE);
		}
	}
}

/**
 * camel_lite_cipher_canonical_to_stream:
 * @part: Part to write.
 ...
 **/
int
camel_lite_cipher_canonical_to_stream(CamelMimePart *part, guint32 flags, CamelStream *ostream)
{
	...
	filter = camel_lite_stream_filter_new_with_stream(ostream);
	canon = camel_lite_mime_filter_canon_new(flags);
	camel_lite_stream_filter_add(filter, canon);
	camel_lite_object_unref(canon);

	if (camel_lite_data_wrapper_write_to_stream((CamelDataWrapper *)part, (CamelStream *)filter) != -1
	    && camel_lite_stream_flush((CamelStream *)filter) != -1)
		res = 0;
