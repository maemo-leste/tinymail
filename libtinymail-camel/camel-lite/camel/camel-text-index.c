...
struct _CamelTextIndexNamePrivate {
	GString *buffer;
	camel_lite_key_t nameid;
	EMemPool *pool;
};

CamelTextIndexName *camel_lite_text_index_name_new(CamelTextIndex *idx, const char *name, camel_lite_key_t nameid);
...
struct _CamelTextIndexCursorPrivate {
	...
	size_t record_count;
	camel_lite_key_t *records;

	char *current;
};

CamelTextIndexCursor *camel_lite_text_index_cursor_new(CamelTextIndex *idx, camel_lite_block_t data);
...
CamelTextIndexKeyCursor *camel_lite_text_index_key_cursor_new(CamelTextIndex *idx, CamelKeyTable *table);
...
struct _CamelTextIndexRoot {
	...
	camel_lite_block_t word_index_root; /* a keyindex containing the keyid -> word mapping */
	camel_lite_block_t word_hash_root; /* a partitionindex containing word -> keyid mapping */

	camel_lite_block_t name_index_root; /* same, for names */
	camel_lite_block_t name_hash_root;
...
struct _CamelTextIndexWord {
	...
	camel_lite_block_t data;	/* where the data starts */
	camel_lite_key_t wordid;
	char *word;
	unsigned int used;
	camel_lite_key_t names[32];
};
...
static CamelObjectClass *camel_lite_text_index_parent;

/* call locked */
static void
text_index_add_name_to_word(CamelIndex *idx, const char *word, camel_lite_key_t nameid)
{
	...
	if (w == NULL) {
		wordid = camel_lite_partition_table_lookup(p->word_hash, word);
		if (wordid == 0) {
			data = 0;
			wordid = camel_lite_key_table_add(p->word_index, word, 0, 0);
			if (wordid == 0){
				g_warning ("Could not create key entry for word '%s': %s\n",
					   word, strerror (errno));
				return;
			}
			if (camel_lite_partition_table_add(p->word_hash, word, wordid) == -1) {
				g_warning ("Could not create hash entry for word '%s': %s\n",
					   word, strerror (errno));
				return;
			}
			rb->words++;
			camel_lite_block_file_touch_block(p->blocks, p->blocks->root_block);
		} else {
			data = camel_lite_key_table_lookup(p->word_index, wordid, NULL, NULL);
			if (data == 0) {
		...
		while (wp && p->word_cache_count > p->word_cache_limit) {
			...
			if (camel_lite_key_file_write(p->links, &ww->data, ww->used, ww->names) != -1) {
				io(printf("  new data [%x]\n", ww->data));
				rb->keys++;
				camel_lite_block_file_touch_block(p->blocks, p->blocks->root_block);
				/* if this call fails - we still point to the old data - not fatal */
				camel_lite_key_table_set_data(p->word_index, ww->wordid, ww->data);
		...
		if (w->used == sizeof(w->names)/sizeof(w->names[0])) {
			io(printf("writing key file entry '%s' [%x]\n", w->word, w->data));
			if (camel_lite_key_file_write(p->links, &w->data, w->used, w->names) != -1) {
				rb->keys++;
				camel_lite_block_file_touch_block(p->blocks, p->blocks->root_block);
				/* if this call fails - we still point to the old data - not fatal */
				camel_lite_key_table_set_data(p->word_index, w->wordid, w->data);
			}
...
text_index_sync(CamelIndex *idx)
{
	...
	if (camel_lite_key_table_sync(p->word_index) == -1
	    || camel_lite_key_table_sync(p->name_index) == -1
	    || camel_lite_partition_table_sync(p->word_hash) == -1
	    || camel_lite_partition_table_sync(p->name_hash) == -1)
		ret = -1;
	...
	ret = camel_lite_block_file_sync(p->blocks);
...
text_index_compress_nosync(CamelIndex *idx)
{
	...
	camel_lite_key_t oldkeyid, newkeyid;
	GHashTable *remap;
	unsigned int deleted;
	camel_lite_block_t data, newdata;
	int i, ret = -1;
	char *name = NULL;
	unsigned int flags;
	char *newpath, *savepath, *oldpath;
	size_t count, newcount;
	camel_lite_key_t *records, newrecords[256];
	struct _CamelTextIndexRoot *rb;
	...
	newidx = camel_lite_text_index_new(newpath, O_RDWR|O_CREAT);
	if (newidx == NULL)
		return -1;
	...
	oldkeyid = 0;
	deleted = 0;
	while ( (oldkeyid = camel_lite_key_table_next(oldp->name_index, oldkeyid, &name, &flags, &data)) ) {
		if ((flags&1) == 0) {
			io(printf("copying name '%s'\n", name));
			newkeyid = camel_lite_key_table_add(newp->name_index, name, data, flags);
			if (newkeyid == 0)
				goto fail;
			rb->names++;
			camel_lite_partition_table_add(newp->name_hash, name, newkeyid);
	...
	oldkeyid = 0;
	while ( (oldkeyid = camel_lite_key_table_next(oldp->word_index, oldkeyid, &name, &flags, &data)) ) {
		...
		while (data) {
			if (camel_lite_key_file_read(oldp->links, &data, &count, &records) == -1) {
				io(printf("could not read from old keys at %d for word '%s'\n", (int)data, name));
				goto fail;
			}
			for (i=0;i<count;i++) {
				newkeyid = (camel_lite_key_t)GPOINTER_TO_INT(g_hash_table_lookup(remap, GINT_TO_POINTER(records[i])));
				if (newkeyid) {
					newrecords[newcount++] = newkeyid;
					if (newcount == sizeof(newrecords)/sizeof(newrecords[0])) {
						if (camel_lite_key_file_write(newp->links, &newdata, newcount, newrecords) == -1) {
							g_free(records);
							goto fail;
						}
						newcount = 0;
		...
		if (newdata != 0) {
			newkeyid = camel_lite_key_table_add(newp->word_index, name, newdata, flags);
			if (newkeyid == 0)
				goto fail;
			camel_lite_partition_table_add(newp->word_hash, name, newkeyid);
		}
		g_free(name);
		name = NULL;
	}

	camel_lite_block_file_touch_block(newp->blocks, newp->blocks->root_block);

	if (camel_lite_index_sync((CamelIndex *)newidx) == -1)
		goto fail;

	/* Rename underlying files to match */
	ret = camel_lite_index_rename(idx, savepath);
	if (ret == -1)
		goto fail;

	/* If this fails, we'll pick up something during restart? */
	ret = camel_lite_index_rename((CamelIndex *)newidx, oldpath);
...
fail:
	...
	camel_lite_index_delete((CamelIndex *)newidx);

	camel_lite_object_unref((CamelObject *)newidx);
	g_free(name);
...
text_index_delete(CamelIndex *idx)
{
	...
	if (camel_lite_block_file_delete(p->blocks) == -1)
		ret = -1;
	if (camel_lite_key_file_delete(p->links) == -1)
		ret = -1;
...
text_index_rename(CamelIndex *idx, const char *path)
{
	...
	ret = camel_lite_block_file_rename(p->blocks, newblock);
	if (ret == -1) {
		CAMEL_TEXT_INDEX_UNLOCK(idx, lock);
		return -1;
	}
	...
	ret = camel_lite_key_file_rename(p->links, newlink);
	if (ret == -1) {
		err = errno;
		camel_lite_block_file_rename(p->blocks, idx->path);
		CAMEL_TEXT_INDEX_UNLOCK(idx, lock);
		errno = err;
		return -1;
	}
...
text_index_has_name(CamelIndex *idx, const char *name)
{
	...
	return camel_lite_partition_table_lookup(p->name_hash, name) != 0;
}
...
text_index_add_name(CamelIndex *idx, const char *name)
{
	...
	keyid = camel_lite_partition_table_lookup(p->name_hash, name);
	if (keyid != 0) {
		/* TODO: We could just update the partition table's
		   key pointer rather than having to delete it */
		rb->deleted++;
		camel_lite_key_table_set_flags(p->name_index, keyid, 1, 1);
		camel_lite_partition_table_remove(p->name_hash, name);
	}

	keyid = camel_lite_key_table_add(p->name_index, name, 0, 0);
	if (keyid != 0) {
		camel_lite_partition_table_add(p->name_hash, name, keyid);
		rb->names++;
	}
	...
	idn = (CamelIndexName *)camel_lite_text_index_name_new((CamelTextIndex *)idx, name, keyid);
...
text_index_write_name(CamelIndex *idx, CamelIndexName *idn)
{
	...
	camel_lite_index_name_add_buffer(idn, NULL, 0);
...
text_index_find(CamelIndex *idx, const char *word)
{
	struct _CamelTextIndexPrivate *p = CTI_PRIVATE(idx);
	camel_lite_key_t keyid;
	camel_lite_block_t data = 0;
	unsigned int flags;
	CamelIndexCursor *idc;

	CAMEL_TEXT_INDEX_LOCK(idx, lock);

	keyid = camel_lite_partition_table_lookup(p->word_hash, word);
	if (keyid != 0) {
		data = camel_lite_key_table_lookup(p->word_index, keyid, NULL, &flags);
		if (flags & 1)
			data = 0;
	}
	...
	idc = (CamelIndexCursor *)camel_lite_text_index_cursor_new((CamelTextIndex *)idx, data);
...
text_index_words(CamelIndex *idx)
{
	...
	return (CamelIndexCursor *)camel_lite_text_index_key_cursor_new((CamelTextIndex *)idx, p->word_index);
}
...
text_index_names(CamelIndex *idx)
{
	...
	return (CamelIndexCursor *)camel_lite_text_index_key_cursor_new((CamelTextIndex *)idx, p->name_index);
}

static void
camel_lite_text_index_class_init(CamelTextIndexClass *klass)
{
	CamelIndexClass *iklass = (CamelIndexClass *)klass;

	camel_lite_text_index_parent = CAMEL_OBJECT_CLASS(camel_lite_type_get_global_classfuncs(camel_lite_object_get_type()));
...
static void
camel_lite_text_index_init(CamelTextIndex *idx)
{
...
static void
camel_lite_text_index_finalise(CamelTextIndex *idx)
{
	struct _CamelTextIndexPrivate *p = CTI_PRIVATE(idx);
	...
	if (p->word_index)
		camel_lite_object_unref((CamelObject *)p->word_index);
	if (p->word_hash)
		camel_lite_object_unref((CamelObject *)p->word_hash);
	if (p->name_index)
		camel_lite_object_unref((CamelObject *)p->name_index);
	if (p->name_hash)
		camel_lite_object_unref((CamelObject *)p->name_hash);

	if (p->blocks)
		camel_lite_object_unref((CamelObject *)p->blocks);
	if (p->links)
		camel_lite_object_unref((CamelObject *)p->links);
...
CamelType
camel_lite_text_index_get_type(void)
{
	static CamelType type = CAMEL_INVALID_TYPE;

	if (type == CAMEL_INVALID_TYPE) {
		type = camel_lite_type_register(camel_lite_index_get_type(), "CamelLiteTextIndex",
					   sizeof (CamelTextIndex),
					   sizeof (CamelTextIndexClass),
					   (CamelObjectClassInitFunc) camel_lite_text_index_class_init,
					   NULL,
					   (CamelObjectInitFunc) camel_lite_text_index_init,
					   (CamelObjectFinalizeFunc) camel_lite_text_index_finalise);
	}
...
CamelTextIndex *
camel_lite_text_index_new(const char *path, int flags)
{
	CamelTextIndex *idx = (CamelTextIndex *)camel_lite_object_new(camel_lite_text_index_get_type());
	struct _CamelTextIndexPrivate *p = CTI_PRIVATE(idx);
	struct _CamelTextIndexRoot *rb;
	char *link;
	CamelBlock *bl;

	camel_lite_index_construct((CamelIndex *)idx, path, flags);
	camel_lite_index_set_normalise((CamelIndex *)idx, text_index_normalise, NULL);

	p->blocks = camel_lite_block_file_new(idx->parent.path, flags, CAMEL_TEXT_INDEX_VERSION, CAMEL_BLOCK_SIZE);
	link = alloca(strlen(idx->parent.path)+7);
	sprintf(link, "%s.data", idx->parent.path);
	p->links = camel_lite_key_file_new(link, flags, CAMEL_TEXT_INDEX_KEY_VERSION);
	...
	if (rb->word_index_root == 0) {
		bl = camel_lite_block_file_new_block(p->blocks);

		if (bl == NULL)
			goto fail;

		rb->word_index_root = bl->id;
		camel_lite_block_file_unref_block(p->blocks, bl);
		camel_lite_block_file_touch_block(p->blocks, p->blocks->root_block);
	}
	...
	p->word_index = camel_lite_key_table_new(p->blocks, rb->word_index_root);
	p->word_hash = camel_lite_partition_table_new(p->blocks, rb->word_hash_root);
	p->name_index = camel_lite_key_table_new(p->blocks, rb->name_index_root);
	p->name_hash = camel_lite_partition_table_new(p->blocks, rb->name_hash_root);
...
int
camel_lite_text_index_check(const char *path)
{
	char *block, *key;
	CamelBlockFile *blocks;
	CamelKeyFile *keys;
	...
	blocks = camel_lite_block_file_new(block, O_RDONLY, CAMEL_TEXT_INDEX_VERSION, CAMEL_BLOCK_SIZE);
	if (blocks == NULL) {
	...
	keys = camel_lite_key_file_new(key, O_RDONLY, CAMEL_TEXT_INDEX_KEY_VERSION);
	if (keys == NULL) {
		io(printf("Check failed: No key file: %s\n", strerror (errno)));
		camel_lite_object_unref((CamelObject *)blocks);
		return -1;
	}

	camel_lite_object_unref((CamelObject *)keys);
	camel_lite_object_unref((CamelObject *)blocks);
...
int
camel_lite_text_index_rename(const char *old, const char *new)
{
	char *oldname, *newname;
	int err;

	/* TODO: camel_lite_text_index_rename should find out if we have an active index and use that instead */
...
int
camel_lite_text_index_remove(const char *old)
{
	char *block, *key;
	int ret = 0;
...
static void
add_type(GHashTable *map, camel_lite_block_t id, int type)
{
	camel_lite_block_t old;
...
static void
add_partition(GHashTable *map, CamelBlockFile *blocks, camel_lite_block_t id)
{
	CamelBlock *bl;
	...
	while (id) {
		...
		bl = camel_lite_block_file_get_block(blocks, id);
		if (bl == NULL) {
		...
		if (pm->used > sizeof(pm->partition)/sizeof(pm->partition[0])) {
			...
			camel_lite_block_file_unref_block(blocks, bl);
			return;
		}
...
static void
add_keys(GHashTable *map, CamelBlockFile *blocks, camel_lite_block_t id)
{
	CamelBlock *rbl, *bl;
	...
	rbl = camel_lite_block_file_get_block(blocks, id);
	if (rbl == NULL) {
	...
	camel_lite_block_file_unref_block(blocks, rbl);
}
...
camel_lite_text_index_dump(CamelTextIndex *idx)
{
	...
	keyid = 0;
	while ( (keyid = camel_lite_key_table_next(p->name_index, keyid, &word, &flags, &data)) ) {
		if ((flags & 1) == 0)
			printf(" %s\n", word);
		else
			printf(" %s (deleted)\n", word);
	...
	keyid = 0;
	while ( (keyid = camel_lite_key_table_next(p->word_index, keyid, &word, &flags, &data)) ) {
		CamelIndexCursor *idc;

		printf("Word: '%s':\n", word);

		idc = camel_lite_index_find((CamelIndex *)idx, word);
		while ( (name = camel_lite_index_cursor_next(idc)) ) {
			printf(" %s", name);
		}
		printf("\n");
		camel_lite_object_unref((CamelObject *)idc);
		g_free(word);
	}
#else
...
camel_lite_text_index_validate(CamelTextIndex *idx)
{
	...
	while ( (keyid = camel_lite_key_table_next(p->word_index, keyid, &word, &flags, &data)) ) {
		...
		while (data) {
			printf(" data %x ", data);
			if (camel_lite_key_file_read(p->links, &data, &count, &records) == -1) {
...
static CamelIndexNameClass *camel_lite_text_index_name_parent;
...
camel_lite_utf8_next(const unsigned char **ptr, const unsigned char *ptrend)
{
...
text_index_name_add_buffer(CamelIndexName *idn, const char *buffer, size_t len)
{
	...
	if (buffer == NULL) {
		if (p->buffer->len) {
			camel_lite_index_name_add_word(idn, p->buffer->str);
	...
	while ((c = camel_lite_utf8_next(&ptr, ptrend))) {
...
static void
camel_lite_text_index_name_class_init(CamelTextIndexNameClass *klass)
{
	CamelIndexNameClass *nklass = (CamelIndexNameClass *)klass;

	camel_lite_text_index_name_parent = CAMEL_INDEX_NAME_CLASS(camel_lite_type_get_global_classfuncs(camel_lite_index_name_get_type()));
...
static void
camel_lite_text_index_name_init(CamelTextIndexName *idn)
{
...
static void
camel_lite_text_index_name_finalise(CamelTextIndexName *idn)
{
	CamelTextIndexNamePrivate *p = CIN_PRIVATE(idn);
...
CamelType
camel_lite_text_index_name_get_type(void)
{
	static CamelType type = CAMEL_INVALID_TYPE;

	if (type == CAMEL_INVALID_TYPE) {
		type = camel_lite_type_register(camel_lite_index_name_get_type(), "CamelLiteTextIndexName",
					   sizeof (CamelTextIndexName),
					   sizeof (CamelTextIndexNameClass),
					   (CamelObjectClassInitFunc) camel_lite_text_index_name_class_init,
					   NULL,
					   (CamelObjectInitFunc) camel_lite_text_index_name_init,
					   (CamelObjectFinalizeFunc) camel_lite_text_index_name_finalise);
	}
...
CamelTextIndexName *
camel_lite_text_index_name_new(CamelTextIndex *idx, const char *name, camel_lite_key_t nameid)
{
	CamelTextIndexName *idn = (CamelTextIndexName *)camel_lite_object_new(camel_lite_text_index_name_get_type());
...
static CamelIndexCursorClass *camel_lite_text_index_cursor_parent;
...
text_index_cursor_next(CamelIndexCursor *idc)
{
	...
	do {
		while (p->record_index >= p->record_count) {
			...
			if (p->next == 0)
				return NULL;
			if (camel_lite_key_file_read(tip->links, &p->next, &p->record_count, &p->records) == -1)
				return NULL;
		}

		g_free(p->current);
		camel_lite_key_table_lookup(tip->name_index, p->records[p->record_index], &p->current, &flags);
		if (flags & 1) {
			g_free(p->current);
			p->current = NULL;
		}
...
static void
camel_lite_text_index_cursor_class_init(CamelTextIndexCursorClass *klass)
{
	CamelIndexCursorClass *cklass = (CamelIndexCursorClass *)klass;

	camel_lite_text_index_cursor_parent = CAMEL_INDEX_CURSOR_CLASS(camel_lite_type_get_global_classfuncs(camel_lite_index_cursor_get_type()));
...
static void
camel_lite_text_index_cursor_init(CamelTextIndexCursor *idc)
{
	CIC_PRIVATE(idc) = g_malloc0(sizeof(struct _CamelTextIndexCursorPrivate));
}

static void
camel_lite_text_index_cursor_finalise(CamelTextIndexCursor *idc)
{
	struct _CamelTextIndexCursorPrivate *p = CIC_PRIVATE(idc);
...
CamelType
camel_lite_text_index_cursor_get_type(void)
{
	static CamelType type = CAMEL_INVALID_TYPE;

	if (type == CAMEL_INVALID_TYPE) {
		type = camel_lite_type_register(camel_lite_index_cursor_get_type(), "CamelLiteTextIndexCursor",
					   sizeof (CamelTextIndexCursor),
					   sizeof (CamelTextIndexCursorClass),
					   (CamelObjectClassInitFunc) camel_lite_text_index_cursor_class_init,
					   NULL,
					   (CamelObjectInitFunc) camel_lite_text_index_cursor_init,
					   (CamelObjectFinalizeFunc) camel_lite_text_index_cursor_finalise);
	}
...
CamelTextIndexCursor *
camel_lite_text_index_cursor_new(CamelTextIndex *idx, camel_lite_block_t data)
{
	CamelTextIndexCursor *idc = (CamelTextIndexCursor *)camel_lite_object_new(camel_lite_text_index_cursor_get_type());
...
static CamelIndexCursorClass *camel_lite_text_index_key_cursor_parent;
...
text_index_key_cursor_next(CamelIndexCursor *idc)
{
	...
	while ( (p->keyid = camel_lite_key_table_next(p->table, p->keyid, &p->current, &p->flags, &p->data)) ) {
		if ((p->flags & 1) == 0) {
			return p->current;
		} else {
			g_free(p->current);
...
static void
camel_lite_text_index_key_cursor_class_init(CamelTextIndexKeyCursorClass *klass)
{
	CamelIndexCursorClass *cklass = (CamelIndexCursorClass *)klass;

	camel_lite_text_index_key_cursor_parent = CAMEL_INDEX_CURSOR_CLASS(camel_lite_type_get_global_classfuncs(camel_lite_index_cursor_get_type()));
...
static void
camel_lite_text_index_key_cursor_init(CamelTextIndexKeyCursor *idc)
{
...
static void
camel_lite_text_index_key_cursor_finalise(CamelTextIndexKeyCursor *idc)
{
	struct _CamelTextIndexKeyCursorPrivate *p = CIKC_PRIVATE(idc);

	g_free(p->current);
	if (p->table)
		camel_lite_object_unref((CamelObject *)p->table);
	g_free(p);
}

CamelType
camel_lite_text_index_key_cursor_get_type(void)
{
	static CamelType type = CAMEL_INVALID_TYPE;

	if (type == CAMEL_INVALID_TYPE) {
		type = camel_lite_type_register(camel_lite_index_cursor_get_type(), "CamelLiteTextIndexKeyCursor",
					   sizeof (CamelTextIndexKeyCursor),
					   sizeof (CamelTextIndexKeyCursorClass),
					   (CamelObjectClassInitFunc) camel_lite_text_index_key_cursor_class_init,
					   NULL,
					   (CamelObjectInitFunc) camel_lite_text_index_key_cursor_init,
					   (CamelObjectFinalizeFunc) camel_lite_text_index_key_cursor_finalise);
	}
...
CamelTextIndexKeyCursor *
camel_lite_text_index_key_cursor_new(CamelTextIndex *idx, CamelKeyTable *table)
{
	CamelTextIndexKeyCursor *idc = (CamelTextIndexKeyCursor *)camel_lite_object_new(camel_lite_text_index_key_cursor_get_type());
	CamelIndexCursor *cic = &idc->parent;
	struct _CamelTextIndexKeyCursorPrivate *p = CIKC_PRIVATE(idc);
	...
	camel_lite_object_ref((CamelObject *)idx);
	p->table = table;
	camel_lite_object_ref((CamelObject *)table);
...
struct _CamelIndexRoot {
	...
	camel_lite_block_t word_root; /* a keyindex containing the keyid -> word mapping */
	camel_lite_block_t word_hash_root; /* a partitionindex containing word -> keyid mapping */

	camel_lite_block_t name_root; /* same, for names */
	camel_lite_block_t name_hash_root;
};
...
int main(int argc, char **argv)
{
	...
	printf("CamelLite text index tester!\n");

	g_thread_init(NULL);
	camel_lite_init(NULL, 0);

	idx = (CamelIndex *)camel_lite_text_index_new("textindex", O_CREAT|O_RDWR|O_TRUNC);


#if 1
	camel_lite_index_compress(idx);
	...
	for (i=0;i<100;i++) {
		...
		idn = camel_lite_index_add_name(idx, name);
		camel_lite_index_name_add_buffer(idn, wordbuffer, sizeof(wordbuffer)-1);
	...
	idc = camel_lite_index_find(idx, "words");
	while ( (word = camel_lite_index_cursor_next(idc)) != NULL ) {
		printf(" name is '%s'\n", word);
	}
	camel_lite_object_unref((CamelObject *)idc);
	printf("done.\n");
	...
	idc = camel_lite_index_find(idx, "truncate");
	while ( (word = camel_lite_index_cursor_next(idc)) != NULL ) {
		printf(" name is '%s'\n", word);
	}
	camel_lite_object_unref((CamelObject *)idc);
...
#if 0
	bs = camel_lite_block_file_new("blocks", "TESTINDX", CAMEL_BLOCK_SIZE);

	root = (struct _CamelIndexRoot *)bs->root;
	if (root->word_root == 0) {
		keyroot = camel_lite_block_file_new_block(bs);
		root->word_root = keyroot->id;
		camel_lite_block_file_touch_block(bs, bs->root_block);
	}
	if (root->word_hash_root == 0) {
		partroot = camel_lite_block_file_new_block(bs);
		root->word_hash_root = partroot->id;
		camel_lite_block_file_touch_block(bs, bs->root_block);
	}

	ki = camel_lite_key_table_new(bs, root->word_root);
	cpi = camel_lite_partition_table_new(bs, root->word_hash_root);
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		keyid = camel_lite_partition_table_lookup(cpi, line);
		if (keyid == 0) {
			m(printf("Adding word '%s' %d\n", line, index));

			keyid = camel_lite_key_table_add(ki, line, index, 0);
			m(printf(" key = %08x\n", keyid));

			camel_lite_partition_table_add(cpi, line, keyid);
		...
		camel_lite_key_table_set_flags(ki, keyid, index, 1);

		data = camel_lite_key_table_lookup(ki, keyid, &key, &flags);
	...
	camel_lite_partition_table_free(cpi);

	printf("Syncing block file\n");
	camel_lite_block_file_sync(bs);
#endif
	return 0;
}
